...
- **轨道 B**（实时，**无 DB**）：同一份库内核被直接消费 —— 当 **Node import**
  导入、走 **MCP server**（stdio，10 个工具 —— 7 个实时 + 3 个读本地 DB）、或通过 **panel HTML**
...
> **更新:** `npx` 会缓存包 —— 要用最新版,重新添加(`claude mcp remove binance-smart-money && claude mcp add …`)或运行 `npx clear-npx-cache`,再重启客户端。
...
你的 AI 就拿到下面 10 个工具（7 个实时 + 3 个读本地 tracker DB）。该 server 是纯 stdio
JSON-RPC，在你调用 DB 类工具（`get_change` / `scan_extreme` / `render_chart`）之前**不加载
